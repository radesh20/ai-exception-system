...
class MCPClient:
    ...
    def __init__(self, base_url: str = ""):
        ...
        logger.info(f"[INFO] MCPClient initialized: {self.mcp_url}")
    ...
    def invoke(self, tool_name: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            logger.debug(f"[DEBUG] [MCPClient] Invoking: {tool_name}")
            response = requests.post(url, json=payload, timeout=30)
            response.raise_for_status()

            result = response.json()
            if result.get("status") == "success":
                logger.info(f"[OK] [MCPClient] {tool_name} succeeded")
                return result.get("result", {})
            else:
                error_msg = result.get("error", "Unknown error")
                logger.warning(f"[WARN] [MCPClient] {tool_name} failed: {error_msg}")
                return {"error": error_msg}

        except requests.RequestException as e:
            logger.error(f"[ERROR] [MCPClient] Request failed: {e}")
            return {"error": str(e)}
